...
export const compressGetIndicatorDataResult = (
	...
	if (result.sma) {
		...
		smaData += `sma data for ${sma_periods.join(', ')} periods:\n`;
	...
	if (result.ema) {
		...
		emaData += `ema data for ${ema_periods.join(', ')} periods:\n`;
	...
	if (result.wma) {
		...
		wmaData += `wma data for ${wma_periods.join(', ')} periods:\n`;
	...
	if (result.rsi) {
		...
		rsiData += `rsi data for ${rsi_periods.join(', ')} periods:\n`;
	...
	if (result.bollingerBands) {
		...
		bbData += `bollinger bands data for ${bb_periods.join(', ')} periods:\n`;
	...
	if (result.atr) {
		...
		atrData += `atr data for ${atr_periods.join(', ')} periods:\n`;
	...
	if (result.volatility) {
		const { periods: volatility_periods, data: volatility_data } =
			result.volatility;
		volatilityData += `volatility data for ${volatility_periods.join(', ')} periods:\n`;
		for (const period of volatility_periods) {
			...
			if (
				volatility_data[period] &&
				Array.isArray(volatility_data[period])
			) {
	...
	if (result.stdDev) {
		...
		stdDevData += `standard deviation data for ${stddev_periods.join(', ')} periods:\n`;
	...
	if (result.slope) {
		...
		slopeData += `slope data for ${slope_periods.join(', ')} periods:\n`;
	...
	if (result.dmi) {
		...
		dmiData += `dmi data for ${dmi_periods.join(', ')} periods:\n`;
	...
	if (result.adx) {
		...
		adxData += `adx data for ${adx_periods.join(', ')} periods:\n`;
	...
	if (result.cci) {
		...
		cciData += `cci data for ${cci_periods.join(', ')} periods:\n`;
	...
	if (result.stochastic) {
		const { configs: stochastic_configs, data: stochastic_data } =
			result.stochastic;
		stochasticData += 'stochastic data\n';
	...
	if (result.stochasticRSI) {
		...
		stochRSIData += 'stochastic rsi data\n';
		...
		for (const { fastKPeriod, fastDPeriod } of stochrsi_configs) {
			...
			if (
				stochrsi_data[dataKey] &&
				Array.isArray(stochrsi_data[dataKey])
			) {
	...
	if (result.averageVolume) {
		const { periods: avgvol_periods, data: avgvol_data } =
			result.averageVolume;
		avgVolData += `average volume data for ${avgvol_periods.join(', ')} periods:\n`;
	...
	if (result.averageVolumeByPrice) {
		...
		avgVolCcyData += `average volume by price data for ${avgvolccy_periods.join(', ')} periods:\n`;
		for (const period of avgvolccy_periods) {
			...
			if (
				avgvolccy_data[period] &&
				Array.isArray(avgvolccy_data[period])
			) {
	...
	if (result.splitAdjustedData) {
		try {
			...
			splitAdjData += `split adjusted data for ${splitadj_periods.join(', ')} periods:\n`;
			...
			for (const period of splitadj_periods) {
				...
				if (
					splitadj_data[period] &&
					Array.isArray(splitadj_data[period])
				) {
