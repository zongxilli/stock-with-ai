...
// 注意: 此API路由已被Server Action替代。
// 相关实现请参见: app/actions/openai/index.ts
// 保留此文件仅用于向后兼容或外部访问。
...
export async function POST(request: Request) {
	try {
		...
		const { symbol, model } = body;

		console.log(`AI Assistant API called for ${symbol} using ${model}`);
		...
		if (model === 'deepseek') {
			return NextResponse.json(mockDeepSeekData);
		} else {
			// Default to OpenAI response
			return NextResponse.json(mockOpenAIData);
		}
	} catch (error) {
